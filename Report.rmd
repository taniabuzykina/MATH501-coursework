...
```{r [H] Preparing data, echo = F}
n_obs <- length(experiment_df$Dose)
n <- experiment_df$Patient_group
...
```{r [H] Performing bayesian inference}
...
```{r [H] Visualising model results - Preparation, echo = F}
...
```{r [H] Visualising model results - Densitiy plots for Beta_0 and Beta_1, echo = F}
...
```{r [H] Visualising model results - Caterpillar plots for Beta_0 and Beta_1, echo = F}
...
```{r [H] Visualising model results - Density and caterpillar plots for p Values, echo = F}
...
```{r [I] Creating the frequentist model}
...
```{r [I] Printing the 95% confidence intervals}
# Print the 95% confidence intervals for beta_0 and beta_1
confint(m_log)
```

```{r [I] Printing the 95% credible intervals}
...
```{r [K] Creating the quadratic model}
...
```{r [K] Performing bayesian inference}
# Performing bayesian inference
...
```{r [K] Printing numerical results of the quadratic model, echo = F}
# Print numerical results of the quadratic model
print(bayesian_binary_logistic_quadratic, intervals = c(0.025, 0.5, 0.975))
```

Looking at the credible intervals for $\beta_2$, we can see that zero IS found within. This is an indication that $\beta_2$ is not an appropriate term for our model.

```{r [K] Printing numerical results of the logarithmic model, echo = F}
# Print numerical results of the non-quadratic model, created in task H
print(bayesian_binary_logistic, intervals = c(0.025, 0.5, 0.975))
```

The results above are from the simple logarithmic model from part (h). We can see that while the credible intervals for $\beta_0$ and $\beta_1$ include zero in the quadratic model, they do not here.

```{r [K] Comparing the DIC (deviance information criterion) values of each model, echo = F}
...
Based on the numerical results above, we can see that the simple model produces a lower DIC value than the quadratic model, which is more favorable. Note that as the values are so similar to one another, it is possible for an execution of this code to result in the quadratic model showing a lower DIC value. Multiple executions have shown that the simple logarithmic model generally produces the lower value.

```{r [K] Visualising model results - Preparation, echo = F}
...
```{r [K] Visualising model results - Density plots, echo = F}
# Density plots
...
The simple model resulted in no signs of posterior support for all unknown values while the quadratic model, with the inclusion of $\beta_2$, resulted in posterior support for all unknown values (not just $\beta_2$).
